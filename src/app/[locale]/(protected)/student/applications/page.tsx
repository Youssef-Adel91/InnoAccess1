...
export default function StudentApplicationsPage() {
    ...
    useEffect(() => {
        ...
        fetchApplications();
    }, [isLoaded, isSignedIn, user, filter, router]);

    const fetchApplications = async () => {
        try {
            const queryParams = filter !== 'all' ? `?status=${filter}` : '';
            const response = await fetch(`/api/student/applications${queryParams}`);
                                    ...
                                    <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                                        ...
                                        <div className="flex-1">
                                            ...
                                            <p className="mt-1 text-gray-600">
                                                {application.jobId.companyId.profile?.companyName ||
                                                ...
                                                <div className="mt-3">
                                                    <details className="group">
                                                        <summary className="text-sm text-blue-600 hover:text-blue-700 cursor-pointer list-none flex items-center">
                                                            {t('viewCoverLetter')}
                                                            <span
                                                                className="ml-1 transform group-open:rotate-180 transition-transform"
                                                                aria-hidden="true"
                                                            >
                                                                ▼
                                                            </span>
